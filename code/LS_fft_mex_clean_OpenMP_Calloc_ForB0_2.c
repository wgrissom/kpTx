 ...
 *
 * compile with 
 * mex -largeArrayDims -lmwlapack CXXFLAGS="$CXXFLAGS -fopenmp" LDFLAGS="$LDFLAGS -fopenmp" COPTIMFLAGS="$COPTIMFLAGS -fopenmp -O2" LDOPTIMFLAGS="$LDOPTIMFLAGS -fopenmp -O2" DEFINES="$DEFINES -fopenmp" LS_fft_mex_clean_OpenMP_Calloc_ForB0.c
        ...
        struct LS   *ls)
{
  ...
  //ls->NhN  = calloc(2*(nC_actual*nNgb)*(nC_actual*nNgb), sizeof(float)); /* 2 for cmplx */
  //ls->NhC  = calloc(2*(nC_actual*nNgb)*nSolve,        sizeof(float));
  ...
  int nrhs, const mxArray *prhs[])
{
  ...
  #pragma omp parallel private(nSeg_t,sh,kTrajInds,nNgb,N,NhN,NhC,coef_t) shared(prhs,grid,shSolve,ls_list,coef_m,nNgbCUMSUM,NRHS) num_threads(NTHREADS)
  {
