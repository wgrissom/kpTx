...
function [F_c, dgrid] = FFTTrick(calib, nComb,densfact)
...
for ii = 1:nComb % huge FFT is too slow in parfor
  ...
  %haha = circshift(fft3(calib0), kshift);
  %F_c{ii}=haha(64/4+1:64/4*3,64/4+1:64/4*3,64/4+1:64/4*3);
  %F_c{ii}=haha(128/4+1:128/4*3,128/4+1:128/4*3,128/4+1:128/4*3);
end
