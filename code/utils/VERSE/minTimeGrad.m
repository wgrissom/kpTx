function g = minTimeGrad(C,g0,gfin,s_gmax,gmax,smax,gamma,T,show)

%minTimeGrad Time Optimal Gradient Design
%  g =  minTimeGrad(C,g0,gfin,s_gmax,gmax,smax,gamma,T,show)
%
...
%
%  This is a modified version of 'minTimeGradient' by Daeho Lee
%  which was orignally written by Michael Lustig.
%
%  [k,g,s] =  minTimeGrad(C,g0,gfin,s_gmax,gmax,smax,gamma,T,show)
%
%  INPUT:
...
%
%
%  Copyright (c) 2005 by Michael Lustig 
%    modified 2006 and 2007
%    modified by Daeho Lee, 2009
...
sta(1) = min(max(g0*gamma+st0), gamma*gmax(1));
p_of_s_half = interp1(s_of_p, [0:dp:Lp-1].', s_half, 'spline');
p_of_s = p_of_s_half(1:2:end);


%% For stability, Daeho %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Duble check this with Miki
if g0 == 0
  gmax(1) = min(gmax(1),gamma*smax*dt);
end
if gfin == 0
  gmax(end) = min(gmax(end),gamma*smax*dt);
end
gmax = interp1(s_gmax, gmax, s_half, 'spline');
if min(gmax) <= 0
  error('wrong gmax: must be positive');
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for n=2:length(s)
    ...
    if mod(n,1000)==1 & show
       s(n)/L*100
       figure(100), plot(s,sta,linspace(s(1),s(end),length(phi)),phi); , axis([0,s(end),0,max(gmax)*gamma*sqrt(2)]);, drawnow
   end
end
...
if isempty(gfin)
    ...
    stb(end) = min(max(gfin*gamma,st0), gamma*gmax(end));
end
...
for n=length(s)-1:-1:1
    ...
    if mod(n,1000)==1 & show
        s(n)/L*100
       figure(100), plot(s,stb,linspace(s(1),s(end), length(phi)),phi,s,sta);, axis([0,s(end),0,max(gmax)*gamma*sqrt(2)]);,drawnow
   end
end
