// Important changes compared to LS_fft_mex_clean_forB0 and LS_fft_mex_clean_OpenMP_SetMaxNhN_Size (no B0)
// In function LS_NhN_form and LS_NhC_form, 
// since we do accumulative sum while reusing the pre-allocated maximum memory for NhN and NhC for different instances 
// we have to reset the content of the memory to all zero before reusing it in differnt instances


/* LS_fft_mex.c
 *
 * compile with
 * mex -largeArrayDims -lmwlapack CXXFLAGS="$CXXFLAGS -fopenmp" LDFLAGS="$LDFLAGS -fopenmp" COPTIMFLAGS="$COPTIMFLAGS -fopenmp -O2" LDOPTIMFLAGS="$LDOPTIMFLAGS -fopenmp -O2" DEFINES="$DEFINES -fopenmp" LS_fft_mex_clean_OpenMP_SetMaxNhNSize_ForB0.c
...
struct LS *LS_ctor(const struct GRID *grid)
{
  ...
  ls->NhN  = mxCalloc(2*(nC_actual*MAXnNgb)*(nC_actual*MAXnNgb), sizeof(float)); /* 2 for cmplx */
  ls->NhC  = mxCalloc(2*(nC_actual*MAXnNgb)*nSolve,        sizeof(float));
  ...
  const struct LS *ls)
{
  ...
  const size_t nCnNgbSq2 = nC_actual*nNgbSq2;  
  ...
  const float       *kTrajInds)
{
  ...
  //Imporatnat:
  //Becuase in SetMaxNhNSize, NhN is reused. So it needs to be cleaned.
  memset(ls->NhN, 0, 2*nC_actual*nNgb*nC_actual*nNgb*sizeof(float));
        ...
        for(CoilQry_i = CoilRef_i; CoilQry_i < nC; CoilQry_i++)
        {
          ...
          for(NgbRef_i = 0; NgbRef_i < nNgb; NgbRef_i++)
          {
            ...
            for(NgbQry_i = 0; NgbQry_i < nNgb; NgbQry_i++)
            {
              ...
              //NhN is allocated with all zeros, so can directly do +=
  ...
  const float       *kTrajInds)
{
  ...
  //Imporatnat:
  //Becuase in SetMaxNhNSize, NhC is reused. So it needs to be cleaned.
  memset(ls->NhC, 0, 2*nC_actual*nNgb*nSolve*sizeof(float));
  ...
  int nrhs, const mxArray *prhs[])
{
  ...
  const size_t nC_actual= (size_t)mxGetScalar(prhs[7]);
  ...
  int nNgbSUM=0;
    ...
    float *coef_m=mxCalloc(2*nNgbSUM*nC_actual*nSolve,sizeof(float));
  ...
  #pragma omp parallel private(nSeg_t,sh,kTrajInds,nNgb,N,NhN,NhC) shared(prhs,grid,shSolve,ls_list,coef_m,nNgbCUMSUM,NRHS) num_threads(NTHREADS)
  {
  ...
  for (nSeg_t = 0; nSeg_t < nSeg; nSeg_t++)
  {
       ...
       {   
          ...
          LS_NhN_form(nNgb, sh, grid, ls_list[tid],BFull,kTrajInds);
          LS_NhC_form(nNgb, nSolve, sh, shSolve, grid, ls_list[tid],BFull,kTrajInds);
          ...
          cposv(&uplo, &N, &NRHS, NhN, &N, NhC, &N, &info);
            ...
            memcpy(coef_m+2*nNgbCUMSUM[nSeg_t]*nC_actual*nSolve, NhC, 2*nC_actual*nNgb*nSolve*sizeof(float));
  ...
  {    
       ...
       if(nNgb != 0)
       {
           coef_t = fort2mat(nC_actual*nNgb, nSolve, nC_actual*nNgb, coef_m+2*nNgbCUMSUM[nSeg_t]*nC_actual*nSolve); 
