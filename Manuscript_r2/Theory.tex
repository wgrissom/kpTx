...
Fourier transform of the \textcolor{blue}{complex-valued}\revbox{R1.1} target k-space excitation pattern $\bm{d}$.
...
in \textcolor{blue}{phase cycles over the excitation field-of-view (cycles/FOV), }\revbox{R1.2} 
...
These inner products can be calculated by interpolating the Fourier transform of the dot product of 
...
\end{equation} 
where the matrix $\bm{S}_l$ is now formed from the $N_c  L$ Fourier transforms of the $B_1^+$ map and $h_l(\vec{x}_j)$ products.} \revbox{R1.3}  
